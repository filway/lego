...
export const testComponents: ComponentData[] = [
  {
    ...
    layerName: '图层1',
    props: {
      ...textDefaultProps, text: 'hello', fontSize: '20px', color: '#000000', lineHeight: '1', textAlign: 'left', fontFamily: '', width: '100px', height: '100px', backgroundColor: '#efefef', left: '100px', top: '150px',
    },
  },
...
const setDirtyWrapper = (callback: Mutation<EditorProps>) => (state: EditorProps, payload: any) => {
  state.isDirty = true;
...
const editor: Module<EditorProps, GlobalDataProps> = {
  ...
  mutations: {
    ...
    updateComponent: setDirtyWrapper((state, {
      key, value, id, isRoot,
    }: UpdateComponentData) => {
    ...
    updatePage: setDirtyWrapper((state, {
      key, value, isRoot, isSetting,
    }) => {
      if (isRoot) {
  ...
  getters: {
    ...
    getElement: (state) => (id: string) => state.components.find((component) => component.id === (id || state.currentElement)),
